let el,ctx,isDrawing=!1,x=0,y=0,isClick=!0;const ChatRoom={init:function(){if(0===$("#chatContent").length)return!1;ChatRoom.editor=Util.newVditor({id:"chatContent",cache:!0,preview:{mode:"editor"},resize:{enable:!1,position:"bottom"},toolbar:["emoji","headings","bold","italic","|","link","upload","|","undo","redo","|","edit-mode",{name:"more",toolbar:["table","list","ordered-list","check","outdent","indent","quote","code","insert-before","insert-after","fullscreen","both","preview","outline","content-theme","code-theme","devtools","info","help"]}],height:150,placeholder:"说点什么吧！",ctrlEnter:function(){ChatRoom.send()}}),ChatRoom.listenUploadEmojis(),ChatRoom.loadEmojis(),$("#emojiBtn").on("click",function(){const t=$("#emojiList");t.hasClass("showList")?t.removeClass("showList"):(t.addClass("showList"),setTimeout(function(){const e=$("body");e.click(function(){t.removeClass("showList"),"aPersonListPanel"!==$(event.target).closest("a").attr("id")&&"personListPanel"!==$(event.target).closest(".module").attr("id")&&($("#personListPanel").hide(),e.unbind())})},100))}),$("#redPacketBtn").on("click",function(){const n=new Map;Util.alert(`<div class="form fn__flex-column">
<label>
...
</div>`,"买定离手"),$("#chipsLabel").on("change",function(){100<$("#chipsLabel").val()&&$("#chipsLabel").val("100"),$("#chipsLabel").val()<=32&&$("#chipsLabel").val("32")})},selectGesture:function(e){Util.alert(`<div class="form fn__flex-column">
...
</div>`,"出拳")},unpackRedPacket:function(o){$.ajax({url:Label.servePath+"/chat-room/red-packet/open",method:"POST",data:JSON.stringify({oId:o,gesture:$("#gestureRadio>input[name=gesture]:checked").val(),dice:{bet:$("#betRadio>input[name=bet]:checked").val(),chips:$("#betRadio>input[name=chips]").val()}}),success:function(t){if(-1!==t.code){let e="";if(void 0!==t.info.gesture)switch(e=t.info.userName+"出拳: ",t.info.gesture){case 0:e+=" 石头";break;case 1:e+=" 剪刀";break;case 2:e+=" 布"}if(Util.alert(`<style>.dialog-header-bg {border-radius: 4px 4px 0px 0px; background-color: rgb(210, 63, 49); color: rgb(255, 255, 255);}.dialog-main {height: 456px;overflow: auto;}</style><div class="fn-hr5"></div>
<div class="fn-hr5"></div>
<div class="fn-hr5"></div>
<div class="fn-hr5"></div>
<div class="ft__center">
    <div class="fn__flex-inline">
        <img class="avatar avatar--small" src="${t.info.userAvatarURL48}" style="background-image: none; background-color: transparent; width: 20px; height: 20px; margin-right: 0px;">
        <div class="fn__space5"></div>
        <a href="${Label.servePath}/member/${t.info.userName}">${t.info.userName}</a>'s 红包
    </div>
    <div class="fn-hr5"></div>
${e?`<div class="ft__smaller ft__fade">${e}</div>`:""}
<div id = "msg" class="ft__smaller ft__fade">${t.info.msg}</div>
...
</ul></div>`,"红包"),ChatRoom.renderRedPacket(t.who,t.info.count,t.info.got,t.recivers,t.diceRet),t.info.count===t.info.got){const a=$("#chatroom"+o);a.find(".hongbao__item").css("opacity",".36").attr("onclick","ChatRoom.unpackRedPacket("+o+")"),!0===t.dice?a.find(".redPacketDesc").html("已开盘"):a.find(".redPacketDesc").html("已经被抢光啦")}}else Util.alert(t.msg)},error:function(e){Util.alert(e.msg)}})},plusOne:function(){ChatRoom.editor.setValue(Label.latestMessage),ChatRoom.send()},renderMsg:function(n,e){let o=!1;var t=Label.latestMessage===n.md;try{var s=$.parseJSON(n.content.replace("<p>","").replace("</p>",""));if("redPacket"===s.msgType){o=!0;let t="未知类型红包",a="ChatRoom.unpackRedPacket('"+n.oId+"')";switch(s.type){case"random":t="拼手气红包";break;case"average":t="普通红包";break;case"specify":t="专属红包";break;case"heartbeat":t="心跳红包 (慎抢)";break;case"rockPaperScissors":t="石头剪刀布红包",s.senderId!==Label.currentUserId&&(a="ChatRoom.selectGesture('"+n.oId+"')");break;case"dice":if(t="摇骰子",s.senderId!==Label.currentUserId){let e=!1;for(idx in s.who)if(s.who[idx].userId===Label.currentUserId){e=!0;break}e||(a="ChatRoom.bet('"+n.oId+"')")}}if(Number(s.count)===Number(s.got)){let e="已经被抢光啦";"dice"===s.type&&(e="已开盘"),n.content=`<div style="opacity: .36;" class="hongbao__item fn__flex-inline" onclick="ChatRoom.unpackRedPacket('${n.oId}')">
<svg class="ft__red hongbao__icon"><use xlink:href="#redPacketIcon"></use></svg>
<div>
    <div>${s.msg}<br><b>${t}</b></div>
    <div class="ft__smaller ft__fade redPacketDesc">${e}</div>
</div>
</div>`}else n.content=`<div class="hongbao__item fn__flex-inline" onclick="${a}">
<svg class="ft__red hongbao__icon"><use xlink:href="#redPacketIcon"></use></svg>
<div>
    <div>${s.msg}<br><b>${t}</b></div>
    <div class="ft__smaller ft__fade redPacketDesc"></div>
</div>
</div>`}}catch(e){}let a="",i="";void 0!==n.userNickname&&""!==n.userNickname?n.userNickname=n.userNickname+" ("+n.userName+")":n.userNickname=n.userName,Label.currentUser===n.userName&&(a=" chats__item--me",i='<a onclick="ChatRoom.revoke('+n.oId+')" class="item">撤回</a>\n'),Label.level3Permitted&&(i='<a onclick="ChatRoom.revoke('+n.oId+')" class="item"><svg><use xlink:href="#administration"></use></svg> 撤回</a>\n');try{var l=n.content.replace("<p>","").replace("</p>","");let t=Util.parseDom(l),a=!1,o="",s=0;for(let e=0;e<t.length;e++){var r=t.item(e);void 0!==r.src&&(a=!0,0!==s&&(o+=","),o+="'"+r.src+"'",s++)}a&&(i+='<a onclick="ChatRoom.addEmoji('+o+')" class="item">一键收藏表情</a>')}catch(e){}let c=`<div class="fn-none">
<div id="chatroom${n.oId}" class="fn__flex chats__item${a}">
<a href="/member/${n.userName}" style="height: 38px">
<div class="avatar tooltipped__user" aria-label="${n.userName}" style="background-image: url('${n.userAvatarURL}')"></div>
</a>
<div class="chats__content">
<div class="chats__arrow"></div>`;l=Label.currentUser!==n.userName||t?"":"display: none;";if(c+=`<div id="userName" class="ft__fade ft__smaller" style="${l}padding-bottom: 3px;border-bottom: 1px solid #eee">
<span class="ft-gray">${n.userNickname}</span>&nbsp;`,void 0!==n.sysMetal&&""!==n.sysMetal){var d=JSON.parse(n.sysMetal).list;if(void 0!==d)for(let e=0;e<d.length;e++){var u=d[e];c+="<img title='"+u.name+" - "+u.description+"' src='"+Util.genMiniMetal(u.attr)+"'/>"}}if(c+="</div>",c+=`<div style="margin-top: 4px" class="vditor-reset ft__smaller ${Label.chatRoomPictureStatus}">${n.content}</div>
<div class="ft__smaller ft__fade fn__right date-bar">${n.time}
...
<details-menu class="fn__layer">
    <a onclick="ChatRoom.at('${n.userName}', '${n.oId}', true)" class="item">@${n.userName}</a>
    ${o?"":`<a onclick="ChatRoom.at('${n.userName}', '${n.oId}', false)" class="item">引用</a>
    <a onclick="ChatRoom.repeat('${n.oId}')" class="item">复读机</a>`}
    <a onclick="ChatRoom.report('${n.oId}')" class="item"><svg><use xlink:href="#icon-report"></use></svg> 一键举报</a>${i}
</details-menu>
</details>
</div></div></div></div>`,e){$("#chats").append(c);let e=$("#chats>div.fn-none");e.show(),e.removeClass("fn-none")}else if(t){let t=++Label.plusN,a=$("#stacked");if(1==t){$("#chats").prepend("<div id='stacked' class='fn__flex' style='position:relative;display:none;'><span id='plusOne' onclick='ChatRoom.plusOne()' style='display:block;margin-left: 20px'><svg style='width: 30px; height: 20px; cursor: pointer;'><use xlink:href='#plusOneIcon'></use></svg></span></div>");let e=$("#chats>div.latest");a.prepend(e),e.find("#userName").show(),e.removeClass("latest")}1!=t&&a.fadeOut(100),setTimeout(function(){a.append(c),a.height(a.height()+27+"px");let e=$("#stacked>div.fn-none");e.show(),e.css("left",9*t+"px"),e.css("top",27*t+"px"),e.css("position","absolute"),e.find(".chats__content").css("background-color",t%2==0?"rgb(240 245 254)":"rgb(245 245 245)"),e.removeClass("fn-none"),a.fadeIn(200)},100)}else{$("#plusOne").remove(),n.md&&(Label.latestMessage=n.md,Label.plusN=0);let e=$("#chats");e.find(".latest").removeClass("latest"),e.prepend(c);let t=$("#chats>div.fn-none");t.slideDown(200),t.addClass("latest"),t.removeClass("fn-none")}},imgViewer:null,imgWaitting:!1,imageViewer:function(){if(!this.imgViewer||$("div.vditor-reset.ft__smaller img:not(.ft__smaller,.emoji)").length!==this.imgViewer.length){this.imgViewer=this.imgViewer||new Viewer(document.querySelector("#chats"),{inline:!1,className:"PWLimgViwer",filter:e=>!e.parentElement.classList.contains("ft__smaller")&&!e.classList.contains("emoji"),title(){let e=this.images[$(".PWLimgViwer .viewer-active").attr("data-index")];for(;e=e.parentElement,!e.querySelector(".avatar"););return"From @"+e.querySelector(".avatar").getAttribute("aria-label")}});const e=function(){return setTimeout(()=>{ChatRoom.imgViewer.isShown?e():(ChatRoom.imgWaitting=!1,ChatRoom.imgViewer.update())},1e3),!0};this.imgWaitting=this.imgWaitting||e()}},iceWs:"",IceGameCK:localStorage.getItem("IceGameCK")||null,loadXiaoIceGame:function(){const e=new WebSocket("wss://game.yuis.cc/wss");ChatRoom.iceWs=e;let t;e.onopen=function(){e.send(JSON.stringify({type:"setUser",user:Label.currentUser,ck:ChatRoom.IceGameCK,uid:Label.currentUserId})),t=setInterval(()=>{e.send(JSON.stringify({type:"hb"}))},15e3)},e.onclose=function(){$("#iceMsgList").prepend('<div class="ice-msg-item"><div class="ice-msg-content">小冰网络失去连接</div></div>')},e.onerror=function(e){$("#iceMsgList").prepend('<div class="ice-msg-item"><div class="ice-msg-content">小冰网络维护中...</div></div>')},e.onmessage=function(e){var t=JSON.parse(e.data);"all"!==t.user&&t.user!==Label.currentUser||(e=`<div class="ice-msg-item">
                    <div class="ice-msg-content">${t.msg}</div>
                  </div>`,$("#iceMsgList").prepend(e)),"setCK"===t.type&&(ChatRoom.IceGameCK=t.ck,localStorage.setItem("IceGameCK",t.ck))},$("#xiaoIceGameBtn").click(function(){$("#xiaoIceGameBox").show(200),$("#xiaoIceGameBtn").hide(200),setTimeout(()=>{$("#xiaoIceGameBox").addClass("active")},220)}),$("#iceClose").click(function(){const e=$("#xiaoIceGameBox");setTimeout(()=>{$(".ice-chat-input").val(""),$("#xiaoIceGameBox").hide(200),$("#xiaoIceGameBtn").show(200)},e.hasClass("active")?420:1),e.removeClass("active")}),$("#iceMinimize").click(function(){$("#xiaoIceGameBox").toggleClass("active")}),$("#iceSendMsg").click(ChatRoom.sendIceMsg),$(".ice-chat-input").bind("keypress",function(e){13===e.keyCode&&(e.preventDefault(),ChatRoom.sendIceMsg())})},sendIceMsg:function(){var e=$(".ice-chat-input").val();$(".ice-chat-input").val("");var t=`<div class="ice-msg-item me"><div class="ice-msg-content">${e}</div></div>`;$("#iceMsgList").prepend(t);let a="gameMsg";/登录/.test(e)&&(a="login"),iceWs.send(JSON.stringify({type:a,ck:ChatRoom.IceGameCK,msg:e}))},loadAvatarPendant:function(){var e=(new Date).getFullYear(),t=(new Date).getMonth()+1,a=(new Date).getDate(),o=e+`-${t}-`+a,s={2021:["2021-02-11","2021-02-17"],2022:["2022-01-31","2022-02-06"],2023:["2023-01-21","2023-01-07"],2024:["2024-02-09","2024-02-15"],2025:["2025-01-28","2025-02-03"],2026:["2026-02-16","2026-01-22"]},n={2021:["2021-09-19","2021-09-21"],2022:["2022-09-10","2022-09-12"],2023:["2023-09-29","2023-10-01"],2024:["2024-09-17","2024-09-19"],2025:["2025-10-06","2025-10-09"],2026:["2026-09-25","2026-09-27"]};let i=document.querySelector("body");10===t&&a<=7?i.classList.add("NationalDay"):12===t&&24<=a&&12===t&&a<=25?i.classList.add("Christmas"):new Date(n[e][0])<=new Date(o)&&new Date(n[e][1])>=new Date(o)?i.classList.add("MidAutumnFestival"):new Date(s[e][0])<=new Date(o)&&new Date(s[e][1])>=new Date(o)&&i.classList.add("SpringFestival")}};