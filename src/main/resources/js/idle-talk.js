        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.code === -1) {
                    ...
                    $("#" + mapId).append("<div id='seek-" + mapId + "' style='background-color: #F7F7F7;display:none;overflow: auto;padding: 20px;'>" + result.data + "<br><button class='btn' style='margin: 20px' onclick='$(\"#" + mapId + "\").remove();'>朕已阅，可以销毁了</button></div>");
                    $("#seek-" + mapId).show(200);
                }
            }
        });
