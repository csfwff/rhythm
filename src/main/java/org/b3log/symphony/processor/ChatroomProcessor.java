    ...
    public synchronized void openRedPacket(final RequestContext context) {
        try {
            ...
            if ("heartbeat".equals(redPacket.getString("type")) || "rockPaperScissors".equals(redPacket.getString("type"))) {
                ...
                if (currentUser.optInt(UserExt.USER_POINT) < (calcRedpacketMax() * 4)) {
                    context.renderJSON(StatusCodes.ERR).renderMsg("抢红包失败！你的账户余额低于 " + (calcRedpacketMax() * 4) + " 积分，不能抢心跳红包和猜拳红包。(积分下限算法：社区用户积分平均数 * 4)");
                    return;
                }
            }
